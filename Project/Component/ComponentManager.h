...
namespace TMF
{
	...
	public:
		...
		void RegisterComponent(std::string name)
		{
			...
			m_addComponentMap.emplace(name, [&](std::weak_ptr<GameObject> pGameObject) 
				{ AddComponent<TComponent>(pGameObject); });
			m_componentNameMap.emplace(typeid(TComponent), name);
			m_removeComponentMap.emplace(name, [&](std::weak_ptr<GameObject> pGameObject,
				int index)
				{ RemoveComponent<TComponent>(pGameObject,index); });
		}
